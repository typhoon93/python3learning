from IPython.display import (
    clear_output,
)  # this function can clear the output so we don't have clutter
...
entered_keys = (
    []
)  # using this to check which keys are entered. we append the entered keys to the list
...
def player_input():
    ...
    while True:
        try:
            player_choice = int(
                input(
                    f"Player {player_turn.upper()}, choose where to place {player_turn.upper()} (1-9): "
                )
            )  # converting input to INT
            if player_choice not in range(
                1, 10
            ):  # user entered a number thats not between 1-9
                print("That's not a valid option, choose a NUMBER between 1 and 9.")
                continue
            elif (
                player_choice in entered_keys
            ):  # user entered a key that was already entered
...
def winning_check(board):  ### CHECK IF A PLAYER IS WINNING
    ...
    elif entered_keys == list(
        range(1, 10)
    ):  # entered keys list already sorted above. Alternatively can use >= 9 as well, instead of creating a new list
...
def board_update(
    player_input,
):  #### UPDATE BOARD FUCNTION#### Updates board with the date entered from the player input function
...
def start_game():  # starts game
    ...
    while winning_check(
        board
    ):  ###IF no winner, func returns TRUE, if there;s a winner it will return False and we will leave the loop
...
def yes_no_game():
    ...
    while True:
        yes_no = input(
            f"Would you like to play a{multiple_games_text} game of Tic Tac Toe? Choose Y or N: "
        )
        if yes_no.lower() == "y":
            start_game()
